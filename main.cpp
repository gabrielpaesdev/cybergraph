...
int main(int argc, char* argv[]) {
    ...
    while (running) {
        ...
        if (appState == STATE_MAIN_MENU) {
            ...
            RenderText(renderer, nullptr, Tr("OPCOES", "OPTIONS"), 400, 80, primaryText, true);
            std::string themeText = isDarkMode ? Tr("Modo Escuro: ON", "Dark Mode: ON") : Tr("Modo Escuro: OFF", "Dark Mode: OFF");
            SDL_Color themeColor = isDarkMode ? GetColorGreen() : GetColorRed();
            RenderText(renderer, nullptr, themeText, 400, 180, themeColor, true);

            std::string soundText = soundEnabled ? Tr("Som: ON", "Sound: ON") : Tr("Som: OFF", "Sound: OFF");
            SDL_Color soundColor = soundEnabled ? GetColorGreen() : GetColorRed();
            RenderText(renderer, nullptr, soundText, 400, 260, soundColor, true);
            ...
            RenderText(renderer, nullptr, "EN", 320, 335, enColor, true);
            RenderText(renderer, nullptr, "PT", 480, 335, ptColor, true);

            RenderText(renderer, nullptr, Tr("Voltar", "Back"), 400, 460, primaryText, true);
        }
